...
fn json_output_only_includes_changed_matched_lines() {
    ...
    let chunks = json["chunks"].as_array().expect("chunks should be an array");
    assert_eq!(chunks.len(), 1);

    let lines = chunks[0].as_array().expect("chunk should be an array of lines");
    assert_eq!(lines.len(), 1, "only changed matched lines should be emitted");
